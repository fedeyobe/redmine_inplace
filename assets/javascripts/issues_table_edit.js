...
$(document).ready(function () {
  ...
  //----------------------------------->
  $(".controller-issues.action-index .list.issues tr:not(.group.open) td").each(function (e) {
    var $this = $(this);
   
    if ($this.hasClass("story_points")) {
        var trClass = $this.closest('tr').attr('class');
        var trTrackerNumber = trClass.match(/tracker-(\d+)/)[1];
        // console.log(trTrackerNumber);
        if (trTrackerNumber === story_points_tracker || story_points_tracker === "") {
    ...
    // Check for other classes
        ...
        // Apply margin-top to span.edit-issue elements that are the only child of their parent td
        $("td > span.edit-issue:only-child").css("margin-top", "-7px");
        $("td > span.td_text:empty").siblings("span.edit-issue").css("margin-top", "-7px");
    ...
    // code for story point
    $(".edit-issue").not(".story_points .edit-issue").on("click", function() {
  ...
  $(".edit-issue").on("click", function (e) {
    ...
    if (clickedtd_split == "project") {
      var temp_this=this;
      appendProjectDropdown(rowId, currentTdClass, projectId, issueId,this);
      var currentTdClass;
      // var currentRow;
    ...
    if (clickedtd_split === "story_points") {
      ...
      // Function to populate the dropdown with story point values
      ...
      // Show text of all cells and hide dropdowns
      $("select[id^='story_points-']").hide();
      $('.td_text').show();

      // Hide text of the clicked cell and show dropdown
      var $tdText = $td.find('.td_text').hide();
      var $dropdown = $td.find("select").css("display", "inline-block");

      var $originalElements = $(this).closest('td').find('.edit-issue').clone(true);
      ...
      $dropdown.on("change", function() {
          ...
          // Update the UI with the new story points value
...
// -------story point end------------//   
      ...
      $(`#${tdId}`).on("change", function () {
        ...
        if (assignedToLink.length > 0) {
          ...
          if (assignedToLink.text().trim() === "") {
            currentRow.find("td.assigned_to span.edit-issue").attr("style", "margin-top: -7px; display:none");
          } else {
            currentRow.find("td.assigned_to span.edit-issue").attr("style", "margin-top: 0px; display:none ");
          }
        } else {
    ...
    //-------------Custom String Type-------------------
      ...
      $.ajax({
        ...
        success: function (data, status, xhr) {
          ...
          toastr["error"]("This custom field is not belong to this project or tracker");
        }
        },
        error: function (xhr, status, error) { 
          $(temp_this).closest("tr").addClass("hascontextmenu");
    ...
    // ----------------Start Date and Due Date------------------
      ...
      var changeDateeee = changeFormat(get_start_date, selected_date_format);
      ...
      $(`#${tdId}`).on("change", function (e) {
        ...
        //         var changeDateFormate = `${dateParts[1]}/${dateParts[2]}/${dateParts[0]}`;
        //         currentRow.find("td.start_date span.td_text").html(changeDateFormate);
        // }else{
        ...
        if (selected_date_format === "YYYY-MM-DD") {
          if (updating_date) {
            ...
            var changeDateFormat = `${dateParts[0]}-${dateParts[1]}-${dateParts[2]}`;
             // currentRow
            //   .find("td.start_date span.td_text")
            //   .html(changeDateFormat);

            var startDateSpan = currentRow.find("td.start_date span.td_text");

          if (startDateSpan.length > 0) {
            startDateSpan.html(changeDateFormat);
            if (startDateSpan.text().trim() === "") {
              currentRow.find("td.start_date span.edit-issue").attr("style", "margin-top: -7px; display:none");
            }
            else {
              currentRow.find("td.start_date span.edit-issue").attr("style", "margin-top: 0px; display:none");
            }
          }
          } else {
            currentRow.find("td.start_date span.td_text").html("");
          ...
          if (updating_date) {
            ...
            var changeDateFormat = dateObj.toLocaleDateString("en-US", options);
             // currentRow
            //   .find("td.start_date span.td_text")
            //   .html(changeDateFormat);
      ...
      var get_due_date = currentRow.find("td.due_date").text().split("\n")[0];
      // console.log(get_due_date, "get_due_date");
      ...
      // console.log(change_dueDate, "change_dueDate");
      ...
      $(`#${tdId}`).on("change", function (e) {
        ...
        if (selected_date_format === "YYYY-MM-DD") {
          if (updating_due_date) {
            ...
            var changeDateFormat = `${dateParts[0]}-${dateParts[1]}-${dateParts[2]}`;
            currentRow.find("td.due_date span.td_text").html(changeDateFormat);
          } else {
            currentRow.find("td.due_date span.td_text").html("");
          }
        } else if (selected_date_format === "DD/MM/YYYY") {
          if (updating_due_date) {
            // console.log(updating_due_date, "updating_due_date");
            var dateParts = updating_due_date.split("-");
            var changeDateFormat = `${dateParts[2]}/${dateParts[1]}/${dateParts[0]}`;
            ...
            if (dueDateSpan.length > 0) {
              ...
              if (dueDateSpan.text().trim() === "") {
                currentRow.find("td.due_date span.edit-issue").attr("style", "margin-top: -7px; display:none");
              } else {
                currentRow.find("td.due_date span.edit-issue").attr("style", "margin-top: 0px; display:none");
              }
            } else {
              currentRow.find("td.due_date span.td_text").html("");
              currentRow.find("td.due_date span.edit-issue").attr("style", "margin-top: -7px; display:none");
        ...
        select_version_id = $("#categorie_issue-" + issueId)
          ...
          if (categorySpan.length > 0) {
            ...
            if (categorySpan.text().trim() === "") {
              currentRow.find("td.category span.edit-issue").attr("style", "margin-top: -7px; display:none");
            } else {
              currentRow.find("td.category span.edit-issue").attr("style", "margin-top: 0px; display:none");
            }
          } 
    ...
    //------------------- issue versions --------------------
      ...
      $(`#${tdId}`).on("change", function () {
        ...
        // console.log(select_version_id, 'id')
        // console.log(select_version, 'name')
        ...
        if (fixedVersionLink.length > 0) {
          fixedVersionLink.html(select_version);
          if (fixedVersionLink.text().trim() === "") {
            currentRow.find("td.fixed_version span.edit-issue").attr("style", "margin-top: -7px; display:none");
          } else {
            currentRow.find("td.fixed_version span.edit-issue").attr("style", "margin-top: 0px; display:none");
          }
        } else {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      for (let i = 0; i < projects.length; i++) {
        projectDropdown +=
          `<option ${issue_project == projects[i].name ? "selected" : ""
          } value = ${projects[i].id}>` +
          projects[i].name +
          "</option>";
      }
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      for (let i = 0; i < priorities.length; i++) {
        priorityDropdown +=
          `<option ${issue_priority == priorities[i].name ? "selected" : ""
...
function trackerDropdown(rowId, currentTdClass, projectId, issueId,temp_this) {
  var tracker_id;
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          for (let i = 0; i < trackers.length; i++) {
            trackerDropdown +=
              `<option ${issue_tracker == trackers[i].name ? "selected" : ""
              } value = ${trackers[i].id}>` +
              trackers[i].name +
              "</option>";
          }
...
function versionDropdown(rowId, currentTdClass, projectId, issueId,temp_this) {
  var version_id;
  $.ajax({
    url: `${url}/issues/${issueId}.json?key=${issue_api_key}`,
    type: "GET",
...
function categorieDropdown(rowId, currentTdClass, projectId, issueId,temp_this) {
  var categorie_id;
  $.ajax({
    url: `${url}/issues/${issueId}.json?key=${issue_api_key}`,
    type: "GET",
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          for (let i = 0; i < categories.length; i++) {
            categorieDropdown +=
              `<option ${issue_categories == categories[i].name ? "selected" : ""
...
function prepareForEdit(temp_this){
  ...
  return {rowId, issueId,tdId,currentTdClass,currentRow,projectId,tdId}
}

$(document).on("click", function(event) {
  if (!$(event.target).closest("select[id^='story_points-']").length &&
